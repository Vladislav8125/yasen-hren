...
export default async function KnowledgePage({ searchParams }: { searchParams: Promise<{ q?: string; section?: string }> }) {
  ...
  return <div className="flex flex-1 flex-col gap-8 p-6 md:p-10">
    <header className="mx-auto w-full max-w-6xl"><p className="font-technical text-xs uppercase tracking-widest text-gold">Научно-исследовательский институт хренологии</p><h1 className="font-display text-4xl text-parchment-hi">НИИХ база знаний</h1><p className="mt-3 max-w-2xl font-body text-lg text-bone-dim">Место, где можно спокойно разобраться, что происходит внутри, как читать карту и куда сделать следующий шаг.</p></header>
