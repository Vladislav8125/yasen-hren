  ...
  return (
    <main ref={landingRef} className="landing-shell overflow-hidden">
      <section className="relative isolate flex min-h-[min(780px,100svh)] flex-col overflow-hidden text-[#fff8e6]">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(25,15,9,.70)_0%,rgba(29,18,10,.48)_52%,rgba(29,18,10,.26)_100%)]" />
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-1 items-center px-5 py-16 md:px-8">
          <div className="max-w-3xl">
            ...
            <Link href="/register" className="mt-9 inline-block rounded bg-red-primary px-14 py-6 font-technical text-sm uppercase tracking-[0.16em] text-[#fff8e6] shadow-lg transition hover:bg-red-primary-dark">Получить карту</Link>
          </div>
        </div>
      </section>
      ...
      <section id="card-demo" className="landing-paper scroll-mt-5 px-5 py-16 md:px-8 md:py-24">
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
          <div>
            ...
            {!firstDrawn && <Link href="/register" className="mt-7 inline-block rounded bg-red-primary px-6 py-3 font-technical text-xs uppercase tracking-widest text-[#fff8e6] hover:bg-red-primary-dark">Получить первую карту</Link>}
          </div>
      ...
      <section id="tariffs" className="landing-paper scroll-mt-6 px-5 py-16 md:px-8 md:py-24">
        <div className="mx-auto max-w-6xl"><p className="font-technical text-xs uppercase tracking-widest text-gold">Тарифы</p><h2 className="mt-3 font-display text-4xl text-parchment-hi md:text-5xl">Выберите глубину, которая нужна сейчас.</h2><p className="mt-4 max-w-2xl font-body text-lg leading-relaxed text-bone-dim">На любом тарифе доступны 35 карт основной колоды, 8 карт Путника и все форматы: веб-приложение, Telegram-бот и VK-бот.</p><div className="mt-10 grid gap-4 lg:grid-cols-3">{plans.map((plan) => <article key={plan.name} className={`landing-plan flex min-h-full flex-col ${plan.highlight ? "landing-plan-featured" : ""}`}><p className="font-technical text-xs uppercase tracking-widest">{plan.note}</p><h3 className="mt-4 font-display text-4xl">{plan.name}</h3><p className="mt-3 font-display text-3xl">{plan.price}</p><ul className="mt-7 space-y-3">{plan.features.map((feature) => <li key={feature} className="font-body text-sm leading-relaxed before:mr-2 before:content-['—']">{feature}</li>)}</ul><Link href={plan.name === "Базовый" ? "/register" : "/tariffs"} className="story-cta mt-auto pt-8"><span>{plan.action}</span><span aria-hidden="true" className="text-xl leading-none">↗</span></Link></article>)}</div></div>
      </section>
